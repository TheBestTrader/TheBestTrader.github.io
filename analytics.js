...
(() => {
  ...
  if (!endpoint || !endpoint.startsWith('https://') || navigator.doNotTrack === '1' || navigator.globalPrivacyControl === true) return;
  try {
    let session;
    const now = Date.now();
    ...
    fetch(endpoint, {method: 'POST', mode: 'cors', credentials: 'omit', keepalive: true, referrerPolicy: 'no-referrer', headers: {'Content-Type': 'text/plain'}, body: payload}).catch(() => {});
  } catch (_) { /* 統計失敗不影響履歷瀏覽。 */ }
})();
